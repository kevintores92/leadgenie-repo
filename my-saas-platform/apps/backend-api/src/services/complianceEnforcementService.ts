...
export class ComplianceEnforcementMiddleware {
  /**
   * Middleware to enforce TCPA compliance requirements
   * Must be called before any call execution
   */
  static async enforceCallCompliance(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const checkboxStates = latestAgreement.checkboxStates as any;
   ...
   */
  static enforcePermittedActions(req: Request, res: Response, next: NextFunction) {
   ...
   */
  static preventMobileColdCalling(req: Request, res: Response, next: NextFunction) {
    const { leads, campaignType } = req.body;

    if (campaignType === 'cold_call') {
      const hasMobile = leads?.some((lead: any) => lead.phoneType === 'mobile');
      if (hasMobile) {
        return res.status(403).json({
   ...
   */
  static async enforceNumberPoolSeparation(req: Request, res: Response, next: NextFunction) {
    try {
